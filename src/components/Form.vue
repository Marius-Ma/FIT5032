...
const validatePassword = (blur) => {
  ...
  if (password.length < minLength) {
    ...
    if (blur) errors.value.password = 'Password must contain at least one lower letter.'
  } else if (!hasNumber) {
...
const validateReason = (blur) => {
  ...
  if (!formData.value.reason) {
    if (blur) errors.value.reason = 'This field is required.'
  } else if (formData.value.reason.length < minLength) {
